...
import { getLearningTrack, getTrackDecisionContent, type LearningTrack } from '../learning-track-policy';

export function toMeTTaSymbol(value: string): string {
...
/** @internal exported so threshold checks can import the Rust contract. */
export const OMEGA_THRESHOLDS = {
  INTENSIVE_MASTERY_MAX: 40,
  GUIDED_MASTERY_MAX: 80,
  INTENSIVE_HINTS_MIN: 2,
} as const;
...
export function evaluateTutoringDecision(state: {
...
}, subjectOrTrack?: string | LearningTrack): TutoringDecision {
  const track = subjectOrTrack ? getLearningTrack(subjectOrTrack) : 'cbc';
  const trackContent = (scaffolding: TutoringDecision['scaffolding']) =>
    getTrackDecisionContent(track, scaffolding);
  ...
  if (
    ...
    const content = trackContent('Intensive');
    return {
      scaffolding: 'Intensive',
      hint: content?.hint ?? 'Let us take one small step together. Look for the part you already know.',
      nextAction: content?.nextAction ?? 'show_conceptual_example',
    };
  }

  // Guided: no attempts yet, or mastery below 80%
  if (state.attempts === 0 || masteryPct < OMEGA_THRESHOLDS.GUIDED_MASTERY_MAX) {
    const content = trackContent('Guided');
    return {
      scaffolding: 'Guided',
      hint: content?.hint ?? 'What do you notice first? Say or write one idea before trying the next step.',
      nextAction: content?.nextAction ?? 'ask_guiding_question',
    };
  }

  // Independent: mastery >= 80%, no frustration, hints < 2
  const content = trackContent('Independent');
  return {
    scaffolding: 'Independent',
    hint: content?.hint ?? 'Try the next question independently, then explain how you got your answer.',
    nextAction: content?.nextAction ?? 'present_next_challenge',
  };
}
